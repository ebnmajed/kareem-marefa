import { getTranslations, setRequestLocale } from "next-intl/server";
import { requireSession } from "@/lib/dal/session";
import { submitCheckInForm } from "./actions";

// SCR-013 — check-in (REQ-CHK-003). One field, one tap from the event page,
// legible enough to fill in quickly while standing in a room.
const KNOWN_ERRORS = new Set(["not_found", "presenter_cannot_check_in", "rate_limited", "not_started", "session_ended", "not_open", "invalid_code", "overlap", "unknown"]);

export default async function CheckInPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string; id: string }>;
  searchParams: Promise<{ success?: string; error?: string }>;
}) {
  const { locale, id } = await params;
  setRequestLocale(locale);
  await requireSession(locale, `/${locale}/app/sessions/${id}/check-in`);
  const { success, error } = await searchParams;
  const t = await getTranslations("checkin");

  const errorKey = error && KNOWN_ERRORS.has(error) ? error : error ? "unknown" : null;

  return (
    <>
      <h1 className="text-h1 text-fg-heading">{t("title")}</h1>
      <p className="mt-2 max-w-prose text-body text-fg-muted">{t("instructions")}</p>

      {success ? (
        <p role="status" className="mt-4 rounded-field border border-edge bg-silver-100 p-3 text-body text-fg-heading">
          {t("success")}
        </p>
      ) : null}
      {errorKey ? (
        <p role="alert" className="mt-4 rounded-field border border-edge-strong p-3 text-body text-fg-heading">
          {t(`error.${errorKey}`)}
        </p>
      ) : null}

      <form action={submitCheckInForm.bind(null, locale, id)} className="mt-8 max-w-sm space-y-4">
        <div>
          <label htmlFor="code" className="text-label text-fg-heading">
            {t("codeLabel")}
          </label>
          <input
            id="code"
            name="code"
            required
            minLength={6}
            maxLength={6}
            autoComplete="off"
            autoCapitalize="characters"
            spellCheck={false}
            inputMode="text"
            dir="ltr"
            className="mt-1 block h-14 w-full rounded-field border border-edge-strong bg-canvas px-4 text-center text-h2 tracking-[0.3em] text-fg-heading uppercase"
          />
        </div>
        <button type="submit" className="inline-flex h-12 w-full items-center justify-center rounded-field bg-navy-950 px-7 text-label text-white hover:bg-navy-900">
          {t("submit")}
        </button>
      </form>
    </>
  );
}
