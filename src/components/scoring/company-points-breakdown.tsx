  ...
  return (
    <section id="company-breakdown" aria-labelledby="company-breakdown-heading" className="mt-12">
      ...
      <p className="mt-2 text-body text-fg-heading">
        {t("balance", { count: breakdown.totalPoints, value: formatNumber(breakdown.totalPoints) })}
      </p>
          ...
          {breakdown.catalogue
            ...
            .map((entry) => (
              <li key={entry.actionKey} className="rounded-field border border-edge p-3 text-body-sm text-fg-muted">
                ...
                {entry.points != null ? (
                  <span> — {t("catalogue.flat", { count: entry.points, value: formatNumber(entry.points) })}</span>
                ) : entry.pointsPerPercent != null && entry.capPoints != null && entry.minActiveMembers != null ? (
                  <span className="flex flex-wrap gap-x-2">
                    <span>
                      {" — "}
                      {t("catalogue.perPercent", { count: Math.round(entry.pointsPerPercent), value: formatNumber(entry.pointsPerPercent) })}
                    </span>
                    <span>{t("catalogue.cap", { count: entry.capPoints, value: formatNumber(entry.capPoints) })}</span>
                    <span>{t("catalogue.min", { count: entry.minActiveMembers, value: formatNumber(entry.minActiveMembers) })}</span>
                  </span>
                ) : null}
              </li>
            ))}
          ...
          {breakdown.rows.map((row) => (
            <li key={row.id} className="rounded-field border border-edge p-3">
              ...
              <p className="mt-1 text-body-sm text-fg-muted">
                ...
                {formatDateTime(row.occurredAt, timeZone, locale)}
              </p>
