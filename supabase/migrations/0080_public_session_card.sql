-- promoted by the lead at post-launch (2026-09-15) · sessions — the public session card (DEC-066: the owner's decision that a shared session link previews). The file below is supabase/proposed/sessions/0001_public_session_card.sql, verbatim; its 03 §8.2 rows are in the header.
...
-- `anon, authenticated` (folded in from proposed/sessions/0002 before promotion went anywhere):
-- /api/s/{id}/og reads the object as WHOEVER asked, and a signed-in member of ANOTHER org is
-- `authenticated`, for whom the org-prefixed exports_storage_read (0037) does not apply. Signing
-- in must never show a person less than signing out would; the predicate is the same door.
