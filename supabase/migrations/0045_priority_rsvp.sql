-- promoted by the lead at wave-2 sync 7 · scoring/0010_priority_rsvp.sql — the priority_rsvp perk's head start
...
begin
  ...
  -- STORY-RSV-005 / OQ-012: the priority window, added at M4. Lead, at
  -- promotion: the window exists only while the org's priority_rsvp perk is
  -- ENABLED — with it off (the seed's default) general RSVP opens at publish,
  -- which is what every M2 flow assumes; with it on, OQ-012's default applies.
  if s.published_at is not null and exists (
       select 1 from public.perks p
        where p.org_id = m.org_id and p.key = 'priority_rsvp' and p.enabled) then
