-- promoted by the lead at wave-2 sync 6 · notify (wave 2, M3) — the Google Calendar connection and the sync jobs.
--
...
language plpgsql security definer set search_path = '' as $$
begin
  -- Lead, at promotion: an org deletion cascades through here with the org and
  -- its members already gone; notify() would raise not_found and abort the
  -- cascade. Nobody is left to tell.
  if not exists (select 1 from public.members m where m.id = old.member_id and m.org_id = old.org_id) then
    return old;
  end if;
