...
| L7 | lead | promotion — `designer`'s roster seed, `branding`'s brand-kit columns, anything proposed — with `db:reset`, RLS, `policy-diff`, the `03` §8.2 rows | invariants 3, 5, 6 | — | — | **in progress** — `0093` (`branding`'s canvasRaise) `e5d5b56`; `0094` (`designer`'s guard walks every colour) and `0095` (`platform_alerts()`) `7daff7f`; `0096` (the platform library reads the roster) and `0097` (a reinstate refuses a requested deletion) `58ce261` — each promotion's live definitions diffed before and after, RLS 76 files / 814 passed at the last, `policy-diff` agrees, trace no gaps. Next: `designer`'s `0002` roster seed and `0003` certificate designs, one reset for both |
...
| P5 | `platform` | `/app/platform/templates` — the platform library, with `DEC-128`'s roster | SCR-083 · `REQ-DSG-008`, `026` | · | `wave8-platform-templates-*.png` | **open** — captures and spec green except the roster case, which asserts the 11 rows (`DEC-148`) and waits on `designer`'s `0002`; re-run and re-capture after that promotion |
