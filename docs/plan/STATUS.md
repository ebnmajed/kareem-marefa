...
| D1 | `designer` | `/app/admin/designer/[documentId]` — mobile view and approve | SCR-057 · `REQ-DSG-005`, `010`, `022`, `DEC-093`, `DEC-096` | · | `wave8-designer-editor-*.png` | **built** — `wave8-designer-editor` green at `5247838`; captures opened: the phone review and read-only states, the 1440 editor; routed: the dynamic-field list headed by plan keys (`session.title`), a render failure in engineer's words, a check with no unit, the live gate's copy. ★ Found by `designer` (`134c563`): **`detach_poster()` had no caller** — an edit to a live poster was saved live and regenerated away; a save to a live poster is now refused and the detach is a confirm naming the session (`REQ-DSG-003`, `REQ-UIX-013`) |
...
| D4 | `designer` | `/app/admin/sessions/[id]/certificates` — review, release, revoke, **and the template chosen at issue time** | SCR-045 · `REQ-CRT-004`, `011`, `DEC-128` | · | `wave8-designer-certificates-*.png` | **built** `bc2b4c5` — `wave8-designer-certificates` and `certificates.spec` green at `5247838`; 9 captures opened: release and revoke confirms name their object, the locked design, the long name wrapping; ★ routed: on a completed session the held certificates sit ~4 phone screens below two design panels — issuance first once any certificate exists. Found by the spec: the eligible list was empty (an ambiguous `check_ins → members` embed swallowed by the DAL) |
