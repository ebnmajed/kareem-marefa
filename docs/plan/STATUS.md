**Last updated:** 2026-09-17 · **Branch:** `wave-9/multi-day` (**draft PR #26**) · **`main`:** **LAUNCHED 2026-09-15; wave 8 merged 2026-09-17** (PR #25, `b7f2f3a`; `0092`–`0099` live on production) · **Phase:** ★★ **WAVE 9 — multi-day sessions (`DEC-119` … `121`, `DEC-150`, `DEC-151`) — THE FOUNDATION IS LANDED, SYNC 1 IS DONE, the five tracks are building.** `0100` + `0101` (the day entity, the two-way derivation of `sessions.starts_at`/`ends_at`/venue, the backfill, the capped ceiling) — ★ **each alone left the entire existing RLS suite green with no test modified.** Five plans read in full; **four design defects caught on paper** (`DEC-151`). **Migrations are additive**, because the owner pushes before merging and `main` runs on the new schema first. ★ **After every merge the owner checks Railway by hand** — its push trigger has never been armed. **Do not start wave 10.**
...
| `tests/unit/admin-audit-labels.test.ts` | `7ed788f` | it reads every `'word.word'` literal in a migration as an audit action, by design; `kareem.days_writer` is a custom Postgres setting, which must contain a dot. Added to the file's own documented exemption list, beside wave 8's `background.color` | no — no assertion touched |
...
| 3 | lead (L4) | a member re-added after the session completed gets their certificate | wave 7's carry: `fan_out_certificates()` fires only into `completed` |
| 4 | `notify` | `{{startsAt}}` in mail is a formatted date, not a raw ISO instant | ★ **conditional on a real Mailpit capture showing the raw string first** |
...
| L4 | lead (custodian) | certificate eligibility reads contract 6 — `fan_out_certificates()`, `issue_certificate()`, `listEligibleRecipients()` — **and `attendance_certificate_sync()`, contract 5's third hook**, which also closes wave 7's carry (named difference 3). After contract 6 is promoted and green | `REQ-SES-017`, `REQ-CRT-001`, `REQ-CHK-017` | ☐ |
