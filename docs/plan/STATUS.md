...
| K5 | `console` | `/app/admin/scoring` — the fixed catalogue and the company rules | SCR-053 · `REQ-PTS-004` … `010`, `REQ-ADM-011` | ✓ (incidental: `ui/combobox`) | `wave8-console-scoring-*.png` | **open, one item** — `wave8-console-scoring` green at `52005ba`; catalogue, penalties (captured with motion reduced), rule-dialog-error opened; ★ the member picker's listbox opens under the fixed tab bar at 390 px, so no option is visible — routed to `console`'s `ui/combobox` |
