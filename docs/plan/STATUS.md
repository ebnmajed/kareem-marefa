**Last updated:** 2026-09-17 · **Branch:** `wave-8/screens` (draft PR #25) · **`main`:** **LAUNCHED 2026-09-15; wave 7 merged 2026-09-16** (PR #24, `4f19cd6`; `0082`–`0091` live on production) · **Phase:** ★★ **WAVE 8 — building.** Task one `e7d0657` (Next 16.3.5, reserve probe 16/16 twice vs a 7/16 control); Step 0 `e3df1d3`; **sync 1 done** (`DEC-148`: four plans approved, contract 3 ruled — a baseline row is a composition, 11 rows, 22 variants); `0092`/`0093` promoted `e5d5b56` (RLS 74 files, 803 passed); **L2 closed** (SCR-043, captures from `d18cc9a`); `0094`–`0097` promoted (`7daff7f`, `58ce261`; RLS 76 files, 814 passed). **Sync 2 (captures from `a4d2886`/`381a05f`):** **B0–B3 closed** (`branding` standing by for D6); **P0–P4, P6, P7 closed**, P5 waits on the seed; `console` K1/K2/K4 opened with findings routed, K3/K5 specs red on strict-mode matches. `designer`'s `0002` roster seed and `0003` certificate designs next, one reset. ★ **Server Action IDs rotate when this wave deploys — the owner deploys outside a scheduled session.** ★ **`0092`–`0097` (and whatever else this wave promotes) are additive and need the owner's rehearsal against a production schema dump, then the push, BEFORE the merge.**
...
| L7 | lead | promotion — `designer`'s roster seed, `branding`'s brand-kit columns, anything proposed — with `db:reset`, RLS, `policy-diff`, the `03` §8.2 rows | invariants 3, 5, 6 | — | — | **in progress** — `0093` `e5d5b56`; `0094`, `0095` `7daff7f`; `0096`, `0097` `58ce261`; **`0098` (the certificate library, 11 platform rows) and `0099` (`ENT-session_certificate_designs`, `certificates.scheme`, `issue_certificate()` from `0088` with three marked changes) `fa93a98`** — every promotion's live definitions diffed before and after; the sweep lists `session_certificate_designs` among the staff-read tables; RLS 79 files, 833 passed; `policy-diff` agrees; trace no gaps. Next: whatever D4/D6 propose |
...
| D2 | `designer` | `/app/admin/templates/posters` | SCR-055 · `REQ-ADM-013`, `REQ-DSG-004`, `007`, `008`, `024`, `026` | · | `wave8-designer-templates-posters-*.png` | **built** `381a05f` — run at `fa93a98`: two spec-side failures (the toast announcer; the empty state in the DOM twice, possibly a DEC-145 orphan) and 7 not run; captures opened — the gradient posters render; routed: lazily-mounted media blank below the fold in full-page captures, the phone library ~9 screens long, the default badge over the logo region |
| D3 | `designer` | `/app/admin/templates/certificates` | SCR-056 · same | · | `wave8-designer-templates-certificates-*.png` | **built** `381a05f` — as D2 (one spec, one page component); the certificate library shows 6 platform rows in both orientations, the light/dark preview choice works; the dark capture's lower cards unmounted, routed |
...
| K1 | `console` | `/app/admin/audit` | SCR-062 · `REQ-ADM-018` | · | `wave8-console-audit-*.png` | **captures opened** at `a4d2886` (filters sheet, filtered admin, filtered moderator) — `admin-audit` green; one low query to `console` (the raw action key on every card) |
| K2 | `console` | `/app/admin/exports` | SCR-061 · `REQ-ADM-017`, `REQ-INT-006` | · | `wave8-console-exports-*.png` | **captures opened** at `a4d2886` (audit note) — `admin-exports` green; routed: the description's «UTF-8 مع BOM» jargon, a card label wrapping |
| K3 | `console` | `/app/admin/reminders` | SCR-060 · `REQ-NTF-*` | · | `wave8-console-reminders-*.png` | **open** — `wave8-console-reminders` red at `a4d2886` on a spec-side strict-mode match (the summary link and the field error share the text); routed |
| K4 | `console` | `/app/admin/recognition` — with the held achievement certificates | SCR-054 · `REQ-REC-*`, `REQ-CRT-012` | · | `wave8-console-recognition-*.png` | **landed** `a4d2886` — `wave8-console-recognition` green, captures opened: held, release-confirm good; ★ **routed:** the manual award's refusal resets the badge select (REQ-UIX-011) and does not name the badge. R-D1 (`releaseAchievements` returns `void`) nudged to `designer` |
| K5 | `console` | `/app/admin/scoring` — the fixed catalogue and the company rules | SCR-053 · `REQ-PTS-004` … `010`, `REQ-ADM-011` | ✓ (incidental: `ui/combobox`) | `wave8-console-scoring-*.png` | **landed** `544ac58` — `wave8-console-scoring` red at `a4d2886` on a spec-side strict-mode match (a `<p>` and a `<bdi>` with the same name); routed; its captures wait on the fix |
