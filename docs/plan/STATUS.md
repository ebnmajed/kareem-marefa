...
| K3 | `console` | `/app/admin/reminders` | SCR-060 · `REQ-NTF-*` | ✓ | `wave8-console-reminders-{field-error,saved}.png` · `wave8-console-reminders.spec.ts` · `5a8f5bc` (verification worktree, `E2E_SHOTS_DIR` → main checkout) | **closed** — opened by the lead: each refused offset said at its own row and in the summary, the units kept; the saved schedule with its toast (`49798f0`, spec `bc17ae5`) |
...
| K5 | `console` | `/app/admin/scoring` — the fixed catalogue and the company rules | SCR-053 · `REQ-PTS-004` … `010`, `REQ-ADM-011` | ✓ (incidental: `ui/combobox`) | `wave8-console-scoring-*.png` | **landed** `544ac58`, `344a921` — `wave8-console-scoring` red at `5a8f5bc`: the deductions still show «يظهر للعضو» twice on a fresh database (the assertion added for the duplicated name); routed |
| K6 | `console` | `/app/admin/emails` — around what it does today, **not** the email studio | SCR-058 · `REQ-ADM-014`, `REQ-NTF-007`, `008` | · | `wave8-console-emails-*.png` | **landed** `ac22709` — run at `5a8f5bc`: the phone case never finds «الموضوع» at `?key=MSG-reminder_1d`; the catalogue capture opened — the matrix and the failure banner read well, the plan's `MSG-*` id on every card routed for removal |
