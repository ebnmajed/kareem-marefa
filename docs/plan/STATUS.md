...
| 7 | Post-launch fixes as a branch → PR → owner merges; close STATUS with the launch record, the final variable inventory and the post-launch list | **in progress** — **PR #19** (`fix/post-launch-navigation`): every feature reachable from the navigation (DEC-064 — eleven URL-only routes wired, the shell one row at 390 px behind a native disclosure), `0078` a check-in code only while live (DEC-064), `0079` walk-ins as a per-session switch, off by default (DEC-065, the owner's decision; `REQ-CHK-010` amended). Gates on the branch: tsc · lint · unit 65/588 · RLS 61/716 · policy-diff · traceability · fresh build + eight e2e specs 59 passed · the event page and the admin dashboard looked at at 390 px. **After the merge: one more gated `supabase db push` for `0078`–`0079`.** Two feature requests await the owner's answers before their teammates are spawned (the owner's standing rule: teammates whenever possible): **points rules for companies** (`scoring` — which events, how much, same board or a column) and **public metadata per session** (`sessions` — a public card page with the poster's `og.png`; the trade-off is that anyone with the link sees title, time and venue, against T3). |
