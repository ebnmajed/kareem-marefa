...
| 7 | Post-launch fixes as a branch → PR → owner merges; close STATUS with the launch record, the final variable inventory and the post-launch list | **in progress** — **PR #19** (`fix/post-launch-navigation`, green): every feature reachable from the navigation (DEC-064), `0078` a check-in code only while live, `0079` walk-ins as a per-session switch (DEC-065). **PR #20** (`post-launch/features`, built by the `sessions` and `scoring` teammates on one branch — the owner's standing rule): **the public session card** (DEC-066, `0080`) and **company points rules** (DEC-067, `0081`); gates on `0e47f1c`: tsc · lint · unit 67/605 · RLS 63 files / 746 (the sweep over 71 entities) · policy-diff · traceability · fresh build · the e2e set green · the card, the scoring screen and the leaderboards looked at at 390 px. **Owed after the merges: one gated `supabase db push` for `0078`–`0081`** (four migrations; the dry run must list exactly those). **Two product readings await the owner** (defaults in force): the percent rules credit every company with a member present, not the host only; `min_active_members = 3`. Known gap: the host company is set from a stopgap form on SCR-053 until the schedule screen (`console`'s) gets the field. Lessons this branch taught, recorded in DEC-066/067 and the notes: the RLS suite is single-runner — two teammates' `applyProposed()` transactions deadlock on one local Postgres, so RLS runs are scheduled by the lead; the local e2e runner does not rebuild; a closed `<details>` keeps a layout box in Chromium; never probe a retyped copy of a regex; `select … into r; if r is not null` is false for a row with any null column. |
