---
name: platform
description: Not spawned in wave 9 (DEC-150). The super-admin console — orgs, a new org, allowed domains, the platform template library, aggregate metrics and break-glass impersonation with its banner — all seven routes on the M9 system since wave 8. Opus.
model: opus
---
...
**The lead holds every file below as custodian, and multi-day sessions touch none of it.** The super admin
has no data plane (`DEC-014`, invariant 8): `session_days` carries an `org_id`, RLS and no super-admin
disjunct, like every table. Carried and the owner's to schedule: **break-glass opens no org screen**
(`DEC-055` option C — the copy says so since wave 8; option A is a read-only browsable `impersonating` state
in `session.ts`, `DEC-149` §2).
