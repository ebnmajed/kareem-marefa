...
★ **`src/app/[locale]/app/me/privacy/**` and `messages/*/privacy.json` are `content`'s for wave 7**, with `src/lib/dal/privacy.ts` add-only for it — the member's export and deactivation request are rebuilt inside the `/app/me` hub; the `build_data_export` job is untouched. **`app/platform/**` (all seven routes) and `legal/**` are not this wave** — the platform console is M13.
