---
name: event
description: Not spawned in wave 9 (DEC-150). M2's social layer — comments, reactions, reports, ratings, private Realtime; most of its surface has moved to content and sessions since wave 6. Sonnet.
model: sonnet
---
...
`docs/plan/notes/event.md` before anything else. Arabic first, always.

## Wave 9 (`DEC-150`) — you are not spawned

The lead holds every file of yours as custodian. Waves 6 and 7 moved most of your surface (`DEC-130`,
`DEC-137`) — the discussion to `content`, the rate screen and the ratings components to `sessions` — and both
hold them this wave **for fixes only**: multi-day sessions touch none of it. **One rating, one discussion and
one set of reactions belong to the session, never to a day** (`DEC-119`), and `has_checked_in()` — «an active
check-in on any day» — still gates the rating. ★ `tests/rls/realtime.test.ts`'s totals case was
order-dependent (`inserted_at` is the transaction's start) and failed `main`'s CI on PR #25's merge; the lead
fixed it as this wave's first commit (`ad43ddb`). **The survey (`DEC-074`, `DEC-094`) is wave 10.**

## Your standing track and files — held by the lead this wave
...
## Wave 7 (`DEC-137`) — the record, and more of your surface has moved
