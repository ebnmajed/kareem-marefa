---
name: event
description: Not spawned in wave 8 (DEC-147). M2's social layer — comments, reactions, reports, ratings, private Realtime; most of its surface has moved to content and sessions since wave 6. Sonnet.
model: sonnet
---
...
The lead holds every file of yours as custodian. Wave 6 and wave 7 moved most of your surface
(`DEC-130`, `DEC-137`) — the discussion to `content`, the rate screen and the ratings components to
`sessions` — and those owners of record stand; neither track is spawned this wave either. **Nothing of
yours is rebuilt, and the survey (`DEC-074`, `DEC-094`) is not this wave.**
