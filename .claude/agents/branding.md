---
name: branding
description: Not spawned in wave 10 (DEC-160). The brand kit — the entity, getBrandKit(), brand_kit() and the org override the four consumers read, mail among them. The lead holds it as custodian. Sonnet.
model: sonnet
---
...
**The lead holds every file below as custodian.** ★ **The email studio is the brand kit's third consumer coming
of age** (`06` §8.3): mail reads three tokens from `public.brand_kit()` today — `fgBody`, `fgMuted`,
`surface` — and eight designed templates will want the logo and the dark palette. **What `notify` needs beyond
the three is a written request to the lead** (contract 9), who makes it in your style: `brand_kit()` stays
`security invoker`, `P1` read, the platform default stays the identity override, and **no hex literal reaches
a template**. The parity goldens do not move for it.
...
- **add-only** `resolveBrand()` in `packages/designer-runtime/src/brand.ts` — the rest of the package is `designer`'s
