...
- `tests/unit/session-matrix.test.ts`, `tests/unit/checkin-*`, `tests/components/checkin/**` **except**
  ...
  `tests/e2e/{checkin,checkin-gating,admin-attendance}.spec.ts`,
...
★ **Sync 1 (`DEC-151`) — what changed for you.** (1) **The ceiling is capped by the next day's start, and
it is the lead's function**: `public.check_in_ceiling(p_day)` (`0101`) — call it in every gate, never copy
it; `resolve_session_day()` already uses it, and nothing having begun it returns the **first** day, so your
gates still answer `not_started`. (2) **Your shadow had a bug**: reopening one day flips
`sessions.check_in_open` false → true, and your session → days trigger would then open **every** day. That
direction acts only for a writer of the **session's** column; the shadow's own recompute marks itself with a
transaction-local setting, **reset after its update**, and the second trigger stands down for it — not
`pg_trigger_depth()`. `0101` already makes a day born with its session's switch. (3) **The attempt rate
limit is per day** (`REQ-SES-015`: «its own rate-limit stream»), a null day counting by session. (4)
**Contract 5 has three hooks**: `attendance_recorded()` / `attendance_removed()` are `scoring`'s and are
**points only**; certificates go through the lead's `attendance_certificate_sync(p_session, p_member)`, and
`remove_check_in()` stops choosing a certificate by `check_in_id`. Until each is promoted your files keep
`main`'s text for it verbatim. (5) `DayWindow.checkInOpen` exists (optional); `SessionDay` carries it.
(6) `rotate_codes` stopping at the ceiling is **named difference 1** — approved, with its own new test.
(7) The two schedule-form tests went to `sessions`.
