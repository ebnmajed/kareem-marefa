---
name: checkin
description: Not spawned in wave 8 (DEC-147). Reservations, the rotating code, the manual check-in switch and its ceiling, the admin's audited removal with its compensating reversal, and the affordance matrix — all on the M9 system since wave 7. Sonnet.
model: sonnet
---
...
teammate's written request. ★ **Wave 7's feature-only transfer of the schedule form ends**:
`admin/sessions/[id]/schedule/**` is the lead's whole this wave, and the walk-in field you added
(`DEC-117`, `DEC-118`) moves with the screen — its strings leave `checkin.json` (`checkin.schedule.*`) for
the lead's new `schedule.json`. `schedule_session()`'s `p_allow_walk_ins` stays **`default null`, meaning
unchanged** (`DEC-141`); a rebuilt form that stops sending it must still never switch walk-ins off.

**Your standing track:** `REQ-RSV-*` and `REQ-CHK-*` — reservations and the waitlist; the rotating
six-character code and its grace window; the attempt rate limit; walk-ins as a publishing setting; the
check-in window read from the schedule — floor `starts_at`, switch `check_in_open` (default open,
`DEC-116`), ceiling `ends_at + 2 h`, and a live state (`DEC-141`); the host view's close/reopen switch;
the admin's removal, which **soft-deletes** (`certificates.check_in_id` is `on delete restrict`) and
writes a compensating `reversal` ledger entry (`reversal:<ledger id>:v1`) and revokes an issued
certificate (`REQ-CHK-017`); and the affordance matrix (`REQ-UIX-015`). **Every reader of `check_ins`
excludes removed rows** unless it is a report that must show them. **Wave 9's multi-day sessions move
check-in to the day** (`DEC-119`) — the day, not the session, will carry the code and the ceiling.

## Your files — held by the lead this wave

- `src/app/[locale]/app/sessions/[id]/{check-in,host}/**` · `src/app/[locale]/app/admin/sessions/[id]/attendance/**`
...
- `tests/unit/session-matrix.test.ts`, `tests/components/checkin/**`, `tests/rls/{rsvp,checkin}*.test.ts`,
  ...
  `tests/e2e/wave7-checkin-*.spec.ts`, `tests/unit/checkin-*`
