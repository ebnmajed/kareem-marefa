---
name: checkin
description: Not spawned in wave 10 (DEC-160). Reservations, the rotating code, per-day check-in, the host view, the admin attendance screen and the affordance matrix — the lead holds them as custodian. Opus.
model: opus
---
...
**The lead holds every file below as custodian, and the wave touches none of it.** `has_checked_in()` — «an
active check-in on any day» — gates the survey exactly as it gates the rating, and it does not change.
`check_in()`, `mark_checked_in_manually()` and `remove_check_in()` still end in contract 5's three hooks; the
third, `attendance_certificate_sync()`, is **`designer`'s from this wave** (`DEC-160`) and gains the re-issue
after a removal — **your functions do not change, and neither does when they call it**.
