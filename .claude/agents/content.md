...
★ **Sync 1 (`DEC-151`) — what changed for you.** (1) **Your grouping showed headings on a ONE-day session**:
it keeps empty buckets for a manager, which at one day is two buckets. And a session cut back from two days
to one may still hold content scoped to the remaining day, which a session-bucket-only list would **hide**.
**With at most one day, render EVERY item flat, whatever its `session_day_id`**; day groups exist only when
there is more than one day to tell apart — `REQ-SES-018`'s own first bullet, cited in the comment. (2)
**`create or replace` with a tenth parameter makes a second overload** and `main`'s worker's nine-argument
call becomes ambiguous: **drop the nine-parameter `record_photo_upload()` in the same file**. (3) The photo
fallback — the day whose nearer edge is closest — is approved; so is `uploaded_at` captured in
`initiate_photo_processing()`. (4) A day-scoped «بعد» material releases when its day has ended **or its
session has** (`completed` / `archived`) — table policy and storage twin alike. (5) **A material's re-scope
is audited** (`material.rescoped`), for the reason `0052` audits its phase — it moves a visibility boundary;
the lead adds the Arabic and English labels in `admin.json`. Tasks and photos: no audit row. (6) `sessions`
counts a day's content in its own query — **publish no `countDayContent()`**. (7) The `<details>` chip is
approved for the first capture; if it clips inside a card or cannot be dismissed from the keyboard, the
lead changes `ui/menu` as custodian.
