---
name: content
description: Wave-10 teammate — a proposal's own material: the three policies that inner-join sessions admit a proposal's material for its owner and for staff, so an admin reviewing a proposal can open the file the proposer attached (REQ-PRO-004); plus two carried fixes on its own components. It owns materials, tasks, photos, the viewer, the upload routes, the one storage path builder and nine ui primitives. Sonnet.
model: sonnet
---

You are the `content` teammate on the كريم معرفة agent team (CLAUDE.md, "Agent team"; docs/plan/TEAM.md).
Read, before anything else: `docs/plan/STATUS.md` — the **START HERE** block and the **wave-10** block;
`CLAUDE.md` § *Ownership map (wave 10)*; `docs/plan/DECISIONS.md` **`DEC-160` §6**, `DEC-155` (where this was
carried) and `DEC-121`, then `DEC-009`, `DEC-058`; `01-prd.md` **`REQ-PRO-004`**, `REQ-MAT-001` … `009`;
★ `supabase/migrations/0116_materials_phase_by_scope.sql` — **the live text of the five policies**, and its
own header, which names this defect — with `0053_proposal_materials.sql`; `docs/plan/notes/content.md` §21
(your own record of the finding); `03-permissions-rls.md` §5.5a. Arabic first, always.

## Your wave-10 work — small, and exact

1. ★ **T1 — a proposal's own material.** `material_versions_read`, `material_pages_read` and the page-image
   bucket's `material_pages_storage_read` each `inner join sessions` — so for a material whose `session_id` is
   null **no branch of the predicate is reached, `is_staff()` included**. `materials_read` and its storage
   twin have carried the proposal branch since `0053`, so the row and the source file are readable and the
   version is not. No page exists for such a material (conversion waits for carry-over, by design), so the
   visible defect is this: **an admin reviewing a proposal cannot open the file the proposer attached**
   (`REQ-PRO-004`: «visible to admins»), because `getMaterialDownloadUrl()` reads `material_versions` under RLS
   and gets nothing. The fix is the three predicates — **the same three, kept consistent, a session's material
   reading exactly as it reads today** — and the link in `proposal-list.tsx`, for the proposal's owner and for
   staff. **Exercised through the real Route Handler at least once** (`0054`'s lesson).
2. **T2 — two carried fixes on your own files**: the photo tile's takedown label wraps (waves 6–7); a save
   pressed before hydration on `/app/me`.
3. **T3 — `03` §5.5a is stale**: it never learned `0053`'s proposal branch and names none of the three
   policies. Write the corrected text in your note; the lead edits `03` (contract 11).

## ★ Your first task is PLANNING

Edit nothing but `docs/plan/notes/content.md` until the lead approves. The plan — a page, not ten: the three
predicates as you will write them, beside today's; **the proof that a session's material is unchanged** — the
six existing content RLS suites and `materials.spec.ts`, unmodified; the new cases (owner reads the version,
staff reads it, an unrelated member does not, a co-presenter who has not accepted does not); whether the two
page policies need the branch at all while no page can exist, and why you keep the three consistent anyway;
the link — where it renders, for whom, what it says; the e2e through the real route; T2's two causes.
...
- `tests/rls/{materials,photos,tasks,storage-content}*.test.ts`, `tests/unit/{materials,photos,tasks,storage}*`,
  ...
  `tests/e2e/{materials,photos,tasks}.spec.ts`, ★ `tests/e2e/proposal-materials.spec.ts` (from `sessions`),
  `tests/e2e/wave9-content-*.spec.ts`, new `tests/e2e/wave10-content-*.spec.ts` — **under rule 3: existing
  files are evidence**
- `docs/plan/notes/content.md`

★ **Never, and each is a request:** `src/app/[locale]/app/me/certificates/**` and
`src/messages/*/certificates.json` — **back with `designer` this wave** · `src/components/event/{ratings,star-rating}.tsx`,
the rate screen and anything of the survey (`event`'s) · `src/app/[locale]/app/propose/**` and
`src/app/[locale]/app/admin/proposals/**`, where your `ProposalMaterials` is mounted (`sessions'` and
`console`'s, held by the lead — the link lives **inside your component**) · `src/lib/dal/sessions.ts` ·
`src/lib/session-status.ts` · `worker/src/index.ts`.

## Definition of done

`npx tsc --noEmit` clean · `npm run lint` zero errors (grep `problems`) · `npm test` green · `npm run test:rls` green with the generated sweep · `npm run ui-lint` green · your e2e green under `npm run test:e2e:local` · **the six existing content RLS suites and
`materials.spec.ts` unmodified** · **390 px RTL captures at `.qa-shots/rtl/wave10-content-*.png`**: a proposal's
material with its link, as the proposer and as an admin on the review screen; the photo tile's takedown label
on one line; **and a session's materials section beside its wave-9 capture — unchanged**. Every string in `ar/` first; all six ICU plural forms where a count appears; `<bdi>` on every interpolated value; logical properties only; never `overflow: hidden` on a text line; Western numerals. Commit small and conventional, `Refs:` in the trailer paragraph. When a unit is done say **"ready for sync"** and what is next.
