...
- `tests/e2e/{sessions-propose,sessions-public-card,sessions-screens,forms-propose,proposal-materials,browse,timeline,event-page,event-rate,leaderboards}.spec.ts`,
  `tests/e2e/wave7-sessions-*.spec.ts`, ★ `tests/e2e/wave8-lead-schedule.spec.ts`, new
...
`src/components/ui/date-time.tsx`, `src/components/admin/**` · `tests/e2e/checkin-schedule-walk-ins.spec.ts`
and `tests/components/checkin/schedule-form.test.tsx` (`checkin`'s — they cover your form's walk-in field; if
your rebuild breaks one, that is a finding) · `transition_session()`'s check-in clause, which is `checkin`'s
hook: you own the function, `checkin` owns what it calls.
